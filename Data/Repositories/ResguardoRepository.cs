...
namespace AppEscritorioUPT.Data.Repositories
{
        ...
        public void Add(Resguardo resguardo)
        {
            ...
            cmd.CommandText = @"
                ...
                (EquipoId, AdministrativoId, ResponsableSistemasId,
                 CodigoInventario, FechaResguardo, Notas, TipoUsoId, FolioLote, TipoResguardo, LaboratorioId)
                VALUES
                (@EquipoId, @AdministrativoId, @ResponsableSistemasId,
                 @CodigoInventario, @FechaResguardo, @Notas, @TipoUsoId, @FolioLote, @TipoResguardo, @LaboratorioId);
            ";
            ...
            cmd.Parameters.AddWithValue("@LaboratorioId", resguardo.LaboratorioId.HasValue ? resguardo.LaboratorioId.Value : DBNull.Value);
    ...
    ";
        ...
        public List<ResguardoReportModel> GetByAdministrativoIdForReport(int administrativoId)
        {
            using var connection = Database.GetOpenConnection();
            using var cmd = connection.CreateCommand();
        ...
        public IEnumerable<Resguardo> GetByAdministrativoId(int administrativoId)
        {
            using var connection = Database.GetOpenConnection();
            using var cmd = connection.CreateCommand();
        ...
        public List<ResguardoReportModel> GetByAreaIdForReport(int areaId)
        {
            using var connection = Database.GetOpenConnection();
            using var cmd = connection.CreateCommand();
            ...
            cmd.CommandText = @"
                ...
                INNER JOIN Administrativos aResp ON aResp.Id = rs.AdministrativoId
                WHERE a.AreaId = @AreaId
                ORDER BY a.NombreCompleto, te.Nombre; -- Ordenamos alfabéticamente por persona y luego por tipo de equipo
            ";
            ...
            while (reader.Read())
            {
                // Reutilizamos la misma estructura de mapeo que ya tienes en tu otro método
                lista.Add(new ResguardoReportModel
                {
                    Id = reader.GetInt32(0),
                    EquipoId = reader.GetInt32(1),
        ...
        public List<ResguardoReportModel> GetByFolioLoteForReport(string folioLote)
        {
            ...
            cmd.CommandText = @"
                ...
                INNER JOIN Administrativos aResp ON aResp.Id = rs.AdministrativoId
                INNER JOIN TiposUso tu ON tu.Id = r.TipoUsoId
                LEFT JOIN Laboratorios lab ON lab.Id = r.LaboratorioId
            ...
            while (reader.Read())
            {
                lista.Add(new ResguardoReportModel
                {
                    ...
                    FechaResguardo = reader["FechaResguardo"]?.ToString() ?? "",
                    Notas = reader["Notas"]?.ToString() ?? "",
                    TipoUsoNombre = reader["TipoUsoNombre"].ToString() ?? "",
